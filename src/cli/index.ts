...
program
  ...
  .description('Deep inspection of a component showing all IR fields and capabilities')
  .option('-f, --from <agent>', 'Source agent (auto-detected if not specified)')
...
program
  ...
  .option('-f, --from <agent>', 'Source agent for both files (auto-detected if not specified)')
  .option('-j, --json', 'Output as JSON')
  .action((sourceA: string, sourceB: string, options: {
    from?: string;
    json?: boolean;
  }) => {
...
program
  ...
  .requiredOption('--via <agent>', `Intermediate agent to convert through (${SUPPORTED_AGENTS.join(', ')})`)
  .option('-f, --from <agent>', 'Source agent (auto-detected if not specified)')
  .option('-j, --json', 'Output as JSON')
