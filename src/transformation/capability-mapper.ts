...
import type { AgentId, CapabilityMapping, MappingStrategy } from '../core/types.js';
...
const mappings: CapabilityMapping[] = [
  // Claude to Windsurf
  {
    ...
    strategy: {
      ...
      description: 'Maps disable-model-invocation: true to auto_execution_mode: 0',
    },
  },
  {
    sourceAgent: 'claude',
  ...
  // Windsurf to Cursor
  {
    ...
    strategy: {
      type: 'fallback',
      fallbackValue: null,
      warning: 'Cursor commands are always manual - auto_execution_mode ignored',
    },
  },
  {
    sourceAgent: 'windsurf',
...
export function getMappings(sourceAgent: AgentId, targetAgent: AgentId): CapabilityMapping[] {
...
export function getCompatibilityMatrix(): Record<AgentId, Record<AgentId, number>> {
  const agents: AgentId[] = ['claude', 'windsurf', 'cursor'];
