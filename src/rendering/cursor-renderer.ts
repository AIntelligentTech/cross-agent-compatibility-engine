...
export class CursorRenderer extends BaseRenderer {
  ...
  render(spec: ComponentSpec, options?: RenderOptions): ReturnType<typeof this.createSuccessResult> | ReturnType<typeof this.createErrorResult> {
    ...
    if (spec.sourceAgent?.id === 'claude') {
      ...
      if (spec.activation.mode !== 'manual') {
        losses.push({
          category: 'activation',
          severity: 'warning',
          description: 'Cursor commands are always manual - auto/suggested activation lost',
          sourceField: 'activation.mode',
          recommendation: 'All Cursor commands require explicit /command invocation',
        });
      }
      ...
      if (spec.invocation.argumentHint) {
        warnings.push({
          ...
          message: 'Cursor has no structured argument system - hint converted to prose',
    ...
    if (spec.intent.whenToUse && spec.intent.whenToUse !== spec.intent.purpose) {
      content += `## When to Use\n\n`;
      content += `${spec.intent.whenToUse}\n\n`;
    }

    // Arguments section (if any)
    if (spec.invocation.argumentHint || (spec.arguments && spec.arguments.length > 0)) {
      content += `## Arguments\n\n`;
      if (spec.invocation.argumentHint) {
    ...
    // Transform the body
    let body = this.transformBody(spec.body, spec);
    content += body;
  ...
  private transformBody(body: string, _spec: ComponentSpec): string {
    ...
    transformed = transformed.replace(/\b(code_search|grep_search|find_by_name|read_file|write_to_file|run_command|browser_preview)\b/g, (match) => {
