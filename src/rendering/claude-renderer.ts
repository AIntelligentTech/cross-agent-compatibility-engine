...
export class ClaudeRenderer extends BaseRenderer {
  ...
  render(spec: ComponentSpec, options?: RenderOptions): ReturnType<typeof this.createSuccessResult> | ReturnType<typeof this.createErrorResult> {
    ...
    if (spec.version.major !== 1 || spec.version.minor !== 0 || spec.version.patch !== 0) {
    ...
    if (spec.sourceAgent?.id === 'windsurf') {
      ...
      if (spec.activation.mode === 'auto' || spec.activation.mode === 'contextual') {
        warnings.push({
          code: 'ACTIVATION_DEGRADED',
          message: 'Windsurf auto_execution_mode mapped to suggested activation',
