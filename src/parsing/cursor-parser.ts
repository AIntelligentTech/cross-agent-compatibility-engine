...
export class CursorParser extends BaseParser {
  ...
  canParse(content: string, filename?: string): boolean {
    ...
    const hasObjective = /^##\s+Objective/mi.test(content);
    const hasRequirements = /^##\s+Requirements/mi.test(content);
  ...
  parse(content: string, options?: ParserOptions): ReturnType<typeof this.createSuccessResult> | ReturnType<typeof this.createErrorResult> {
    ...
    const id = this.extractIdFromFilename(options?.sourceFile) ?? this.slugify(title);
    ...
    const description = fm.description ?? this.extractSection(body, 'Objective') ?? title;
    ...
    warnings.push('Cursor commands have no structured argument system - arguments are free-form');
  ...
  private inferCapabilities(body: string): CapabilitySet {
    ...
    caps.needsShell = lowerBody.includes('terminal') || lowerBody.includes('command') || lowerBody.includes('shell') || lowerBody.includes('run');
