...
export class ClaudeParser extends BaseParser {
  ...
  canParse(content: string, filename?: string): boolean {
    if (filename) {
      if (filename.includes('.claude/skills/') || filename.includes('.claude/commands/')) {
        return true;
      }
    }
  ...
  parse(content: string, options?: ParserOptions): ReturnType<typeof this.createSuccessResult> | ReturnType<typeof this.createErrorResult> {
    ...
    const id = fm.name ?? this.extractIdFromFilename(options?.sourceFile) ?? 'unknown-skill';
    ...
    if (fm.context === 'fork') {
      warnings.push('Claude "fork" context has no direct equivalent in other agents');
    }
    if (fm.agent) {
  ...
  private inferCapabilities(body: string, allowedTools?: string[]): CapabilitySet {
    const caps = createDefaultCapabilities();
    const lowerBody = body.toLowerCase();

    // Infer from body content
    caps.needsShell = lowerBody.includes('terminal') || lowerBody.includes('command') || lowerBody.includes('shell');
    caps.needsGit = lowerBody.includes('git') || lowerBody.includes('commit') || lowerBody.includes('branch');
    caps.needsNetwork = lowerBody.includes('http') || lowerBody.includes('api') || lowerBody.includes('fetch');
    caps.needsBrowser = lowerBody.includes('browser') || lowerBody.includes('screenshot');
    caps.needsCodeSearch = lowerBody.includes('search') || lowerBody.includes('find') || lowerBody.includes('grep');

    // Infer from allowed tools
    if (allowedTools) {
      for (const tool of allowedTools) {
        if (tool.includes('bash') || tool.includes('shell')) caps.needsShell = true;
        if (tool.includes('git')) caps.needsGit = true;
        if (tool.includes('browser')) caps.needsBrowser = true;
        if (tool.includes('search') || tool.includes('grep')) caps.needsCodeSearch = true;
      }
    }

    // Infer what it provides
    caps.providesAnalysis = lowerBody.includes('analyz') || lowerBody.includes('review') || lowerBody.includes('audit');
    caps.providesCodeGeneration = lowerBody.includes('implement') || lowerBody.includes('create') || lowerBody.includes('generate');
